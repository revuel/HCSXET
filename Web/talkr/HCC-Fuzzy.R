...
#   install.packages('FuzzyToolkitUoN', repos='http://cran.us.r-project.org')
# } 
...
tryCatch( {
  ...
  plot(vector, type="o", col="blue", xlab="# Cuestión", ylab="Puntuación media", 
       main="Valores de respuesta",
       xlim=c(1, 15))
...
tryCatch( {
  ...
  names <- c("armonia", "contexto", "disposicion", "intercesion", "custodia")
  datos <- vector.avg
  datos.names <- names
...
# Add variables
hcFIS <<- addVar(hcFIS, "input", "Armonía", 10:30)
hcFIS <<- addVar(hcFIS, "input", "Contexto", 10:30)
hcFIS <<- addVar(hcFIS, "input", "Disposición", 10:30)
hcFIS <<- addVar(hcFIS, "input", "Intercesión", 10:30)
hcFIS <<- addVar(hcFIS, "input", "Custodia", 10:30)
hcFIS <<- addVar(hcFIS, "output", "Puntuación", 1:10)

# Create and add Input Memebership functions (Armonía)
...
# Create and add Input Memebership functions (Disposición)
