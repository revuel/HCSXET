<!DOCTYPE html>
<div class = "container">
	<hr>
	<div class="row">
		<div class="col-xs-6 col-md-4 col-sm-4">
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title text-center">Referencias</h3>
				</div>
				<div class="panel-body">
					<a href="personalidades.php"><p class="text-center">Personalidades</p></a>
				</div>
				<div class="panel-body">
					<a href="instituciones.php"><p class="text-center">Instituciones</p></a>
				</div>
				<div class="panel-body">
					<a href="articulos.php"><p class="text-center">Artículos académicos</p></a>
				</div>
			</div>
		</div>
		<div class="col-xs-6 col-md-4 col-sm-4 text-center">
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title">Universidad Francisco de Vitoria</h3>
				</div>
				<div class="panel-body">
					<a href="http://www.ufv.es/inicio" target="_blank">
					<img class ="img-responsive" src="Pics/logoufv.png" alt="Logo UFV" ismap>
					</a>
				</div>
			</div>
		</div>
		<div class="col-xs-6 col-md-4 col-sm-4">
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title text-center">Acerca de</h3>
				</div>
				<div class="panel-body">
					<a href=""><p class="text-center">Autores</p></a>
				</div>
				<div class="panel-body">
					<a href=""><p class="text-center">Contactar</p></a>
				</div>
				<div class="panel-body">
					<a href=""><p class="text-center">Repositorio</p></a>
				</div>
			</div>
		</div>
	</div>
	<hr>
</div>